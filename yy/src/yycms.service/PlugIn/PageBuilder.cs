...
namespace yycms.service.PlugIn
{
	public class PageBuilder
	{
			...
			if (!String.IsNullOrEmpty(PageCode))
			{
				...
				if (pagers != null && pagers.Count > 0)
				{
					foreach (var p in pagers)
					{
						...
						if (p.Attributes["total"] == null)
						{ return; }
						if (p.Attributes["modal"] == null)
						{ return; }
						#endregion 必须属性检查
						...
						if (!Int64.TryParse(Attrs["pagesize"].Value, out PageSize))
						{ return; }
						try
						{
		...
		public String BuildPage(String PageCode, Int64 DataID)
		{
			...
			for (Int32 i = 0; i < Tags.Count; i++)
			{
				if (!Tags[i].HasAttributes || Tags[i].Attributes["modal"] == null)
				{ continue; }
				#region View
				...
				if (!String.IsNullOrEmpty(View))
				{
					...
					#endregion 如果模板在指定ID的元素里,就替换InnerHtml
					...
					#endregion 如果模板在YYTag标签里,就替换到当前YYTag后面
		...
		protected String GetPageSavePath(Int64 PageID, Int64 PageIndex, Int64 DataID)
		{
			...
			{ SavePath = "/"; }
			else if (SavePath.IndexOf("~") == 0)
			{ SavePath = SavePath.Substring(1); }

			if (String.IsNullOrEmpty(ExtensionName))
			{ ExtensionName = "{0}.html"; }
			...
			if (String.IsNullOrEmpty(Path.GetExtension(SaveFilePath)))
			{ SavePath += ".html"; }
