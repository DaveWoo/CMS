...
namespace yycms.union.wechat
{
	public class RedPack : SDK
	{
			...
			String nonce_str, String mch_key, String mch_certPath)
		{
			...
			if (!String.IsNullOrEmpty(mch_billno))
			{ param.Add("mch_billno", mch_billno); }
			if (!String.IsNullOrEmpty(mch_id))
			{ param.Add("mch_id", mch_id); }
			if (!String.IsNullOrEmpty(wxappid))
			{ param.Add("wxappid", wxappid); }
			if (!String.IsNullOrEmpty(nick_name))
			{ param.Add("nick_name", nick_name); }
			if (!String.IsNullOrEmpty(send_name))
			{ param.Add("send_name", send_name); }
			if (!String.IsNullOrEmpty(re_openid))
			{ param.Add("re_openid", re_openid); }
			if (!String.IsNullOrEmpty(total_amount))
			{ param.Add("total_amount", total_amount); }
			if (!String.IsNullOrEmpty(min_value))
			{ param.Add("min_value", min_value); }
			if (!String.IsNullOrEmpty(max_value))
			{ param.Add("max_value", max_value); }
			if (!String.IsNullOrEmpty(total_num))
			{ param.Add("total_num", total_num); }
			if (!String.IsNullOrEmpty(wishing))
			{ param.Add("wishing", wishing); }
			if (!String.IsNullOrEmpty(client_ip))
			{ param.Add("client_ip", client_ip); }
			if (!String.IsNullOrEmpty(act_name))
			{ param.Add("act_name", act_name); }
			if (!String.IsNullOrEmpty(remark))
			{ param.Add("remark", remark); }
			if (!String.IsNullOrEmpty(logo_imgurl))
			{ param.Add("logo_imgurl", logo_imgurl); }
			if (!String.IsNullOrEmpty(share_content))
			{ param.Add("share_content", share_content); }
			if (!String.IsNullOrEmpty(share_url))
			{ param.Add("share_url", share_url); }
			if (!String.IsNullOrEmpty(share_imgurl))
			{ param.Add("share_imgurl", share_imgurl); }
			if (!String.IsNullOrEmpty(act_id))
			{ param.Add("act_id", act_id); }
