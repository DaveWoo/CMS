...
namespace yycms.union.wechat
{
	public class Ticket : SDK
	{
			...
			long? _end_timestamp)
		{
			...
			if (!String.IsNullOrEmpty(_logo_url))
			{ BaseInfo["logo_url"] = _logo_url; }
			//if (!String.IsNullOrEmpty(_color)) { BaseInfo["color"] = _color; }
			if (!String.IsNullOrEmpty(_notice))
			{ BaseInfo["notice"] = _notice; }
			if (!String.IsNullOrEmpty(_service_phone))
			{ BaseInfo["service_phone"] = _service_phone; }
			if (!String.IsNullOrEmpty(_description))
			{ BaseInfo["description"] = _description; }
			if (_location_id_list != null)
			{ BaseInfo["location_id_list"] = JsonConvert.SerializeObject(_location_id_list); }
			if (!String.IsNullOrEmpty(_custom_url_name))
			{ BaseInfo["custom_url_name"] = _custom_url_name; }
			if (!String.IsNullOrEmpty(_custom_url))
			{ BaseInfo["custom_url"] = _custom_url; }
			if (!String.IsNullOrEmpty(_custom_url_sub_title))
			{ BaseInfo["custom_url_sub_title"] = _custom_url_sub_title; }
			if (!String.IsNullOrEmpty(_promotion_url_name))
			{ BaseInfo["promotion_url_name"] = _promotion_url_name; }
			if (!String.IsNullOrEmpty(_promotion_url))
			{ BaseInfo["promotion_url"] = _promotion_url; }
			if (!String.IsNullOrEmpty(_promotion_url_sub_title))
			{ BaseInfo["promotion_url_sub_title"] = _promotion_url_sub_title; }
			if (!String.IsNullOrEmpty(_code_type))
			{ BaseInfo["code_type"] = _code_type; }
			if (_get_limit.HasValue)
			{ BaseInfo["get_limit"] = _get_limit.Value; }
			if (_can_share.HasValue)
			{ BaseInfo["can_share"] = _can_share.Value; }
			if (_can_give_friend.HasValue)
			{ BaseInfo["can_give_friend"] = _can_give_friend.Value; }
			if (_type.HasValue)
			{ BaseInfo["type"] = _type.Value; }
			if (_begin_timestamp.HasValue)
			{ BaseInfo["begin_timestam"] = _begin_timestamp.Value; }
			if (_end_timestamp.HasValue)
			{ BaseInfo["end_timestamp"] = _end_timestamp.Value; }
